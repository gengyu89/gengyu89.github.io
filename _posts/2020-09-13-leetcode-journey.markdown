...
In geological sciences or other academic fields related to scientific computation, many people believe that the programming they learned is close or similar to what people are doing in Computer Science (CS), which is the main reason I am bothering myself explaining the basics. The truth is, most job in geophysics major is about invoking finite element libraries through Application Programming Interface (API) without worrying about lower-level details, which is the main reason people think programming is (much) easier than the interpretation of results.

Annoyingly, after so many years studying in geophysics, I found the programming in this major boring, or I can even use the word "lame" to describe it. In my opinion, the standard about programming is too low and when people state "similar", they probably didn't understand the fact in this major.

Our first discussion question must be, *Why do I consider geophysicists incapable of developing software?*, or *Why are programming more difficult than what most people think?*.

**Why are data structures and algorithms so important?**

Software development, in real cases, relies on a variety of comprehensive skills. "Design patterns for solving algorithmic problems" - like many university programs taught us - are ideal abstractions of real-world problems. Merging software modules by multiple developers, compiling and fixing a package in your current environment, or for some reason remaking something from the scratch, etc. will definitely bring much more pain than designing and implementing algorithms.
...
From the micro-level, concepts like graph theory and discrete mathematics used by computer scientists determine how objects/events can be located in memories and how each record is related to its neighbors, which usually results in an impact on execution efficiencies. Although this may not matter for small problems, the effect accumulates and eventually becomes significant as a project grows larger. Concerns about these aspects make CS to some extent - although not totally - different from other subjects that require code writing skills.
...
From the other aspect, it is much more difficult to provide a list of "essential algorithms". There are quite a tons of them and the number of algorithms is growing continually (the more you know, the better).

Nevertheless, exquisite algorithms you come up with by afflatus must be built upon fundamental ones - the ones that are considered classical and the ones you must be familiar even not in the situation of a coding contest. Some of those are listed here:
...
* What is the definition of "average probing length"? Note: this is given by Donald Knuth.
