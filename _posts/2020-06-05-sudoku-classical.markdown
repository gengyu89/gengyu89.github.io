...
经我鉴定，该大牛之所谓 “编程爱好” 其实也就是说说而已，没怎么落实到行动上。恕在下直言一点事实，不带有任何偏见：**稍微做过一点调查的人，都不会得出这样的结论来**。Computer Scientists 研究的内容完全就跟地球物理学家两码事，这种差别绝非是普通人经过一两个月的自学能弥补（跨行业申请的难度，比一般人想象得要高）的。当程序员恐怕是行不了，真有这样的志向，从现在起一点一滴地积累才行得通呢！
...
在加拿大攻读计算机科学辅修学位，翻看过数学家 Kenneth H. Rosen 的数论教材，研究过包括 SICP 在内的计算机科学经典教材并因此而学了函数式编程（强制你形成递归的思维），见识过各种各样牛逼的解题方法… Common-Lisp, Java, Python... 是我那时为了读算法书而学的。知道这个圈子的水有多深，对于这个领域内的各种出题套路和比赛规则，我都非常了解。有时听到一些周围的人说自己 “喜欢编程”，我总感觉非常幼稚。
...
在我看来，ACM 一类的比赛尽管存在争议，对于大学生的程序设计能力是一种良好的训练。软件是由模块组成的 —— 不同人员开发的代码借由 API 相互调用、协同运作，只有当每一个部件都优化到位，才能将整体的性能发挥到极致。野路子上来的程序员表面上看起来什么都懂，内功拼起来差得很远。考虑到硬件的更新速度赶不上数据量的增长，从这个角度来讲，算法比赛和研究的意义始终不容忽略。我虽代表不了这个学科的最高水平，希望以数独为例，引出算法比赛里的一些非常基础的优化方法。
