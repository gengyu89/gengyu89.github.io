...
广义地讲，我承认 “熟练地使用终端命令、脚本语言和入门水准的 C++ 完成数据处理” 是地球物理一类的专业必会技能，并同意把这些也纳入编程的范畴。遗憾的是，**这些技术都没有触及 CS 的核心内容**。在我看来，ACM 一类的比赛尽管存在争议，对于大学生的程序设计能力是一种良好的训练。软件是由模块组成的 —— 不同人员开发的代码借由 API 相互调用、协同运作，只有当每一个部件都优化到位，才能将整体的性能发挥到极致。考虑到硬件的更新速度赶不上数据量的增长，从这个角度来讲，算法比赛和研究的意义始终不容忽略。我虽代表不了这个学科的最高水平，希望以数独为例，引出算法比赛里的一些非常基础的优化方法。
