...
经我鉴定，该大牛之所谓 “编程爱好” 其实也就是说说而已，没看出有多大的热情。恕在下直言一点事实，不带有任何偏见：**稍微做过一点调查的人，都不会得出这样的结论来**。毫不留情地说，Computer Scientists 研究的内容基本上就是完全不同的概念，这种差别绝非是普通人经过一两个月的自学能够弥补（跨行业申请的难度，比一般人想象得要高）的。恕我直言，地球物理这个专业的人毕业后想干和编程相关的工作，还得从基本原理学起！

> Design Patterns 都不懂，又怎么好意思说自己会编程序呢？

#### —— 基本理论和性质 ——

开博第一篇，先跟大家交待自己的学习背景。

在加拿大攻读计算机科学辅修学位，翻看过数学家 Kenneth H. Rosen 的数论教材，研究过包括 SICP 在内的计算机科学经典教材并因此而学了函数式编程（强制你形成递归的思维），见识过各种各样牛逼的解题方法… Common-Lisp, Java, Python… 是我那时为了读算法书而学的。知道这个圈子的水有多深，对于这个领域内的各种出题套路和比赛规则，我都稍有了解。有时听到一些周围的人说自己 “喜欢编程”（吴果、张佳、伊丽莎白… 什么的），我总感觉非常幼稚。
...
如果说要学好一门技能，九成靠的是钻研和自学，那往往这 10% 的引导和启发是最重要的。我那时所接触到的程序设计方法对我至今影响深刻，虽然这些训练只能让我成为一个新手，好在我走的是以算法为核心的正规化学习路线。Computer Science 如同纯数学，有推导有结论，是一门**理论性很强的研究型学科**。大多数国外大学的 CS 专业会把教学精力集中于算法，编程语言作为工具，需要自己去学。外行人眼中 “教会你熟练使用各种编程语言” 之理解，近乎和 “能用最基础的方法和最少量的公式还原一个三阶魔方” 差不多肤浅。

广义地讲，“熟练地使用终端命令、脚本语言和入门水准的 C++ 完成数据处理” 是地球物理一类的学科之必会技能。如果把这些也纳入编程的范畴的话，遗憾的是，这些技术都没有触及 CS 的核心内容。在我看来，ACM 一类的比赛尽管存在争议，对于大学生的程序设计能力是一种良好的训练。软件是由模块构成的 —— 不同人员开发的代码借由 API 相互调用、协同运作，只有当每一个部件都优化到位，才能将整体的性能发挥到极致。野路子上来的程序员表面上看起来什么都懂，内功拼起来差得很远。考虑到硬件的更新速度赶不上数据量的增长，从这个角度来讲，算法比赛和研究的意义始终不容忽略。我虽代表不了这个学科的最高水平，希望以数独为例，引出算法比赛里的一些非常基础的优化方法。
...
链表是除数组以外最先需要考虑的数据结构，也是**应用程序开发最简单的加速方法之一**。双向链表当中，每一个成员包含一个值和两个指向相邻成员的指针，可首尾相连形成环状结构；要想删除某一个成员，只需修改前、后两个成员的指针使得这个被删除成员在遍历时被跳过，便于对已删除成员快速恢复，因而更适合需要频繁删除和插入的操作。
```python
import collections
...
可见编程其实很难，难在：(0) 选用什么样的数据结构，在缺少背景知识的前提下，仅凭临场发挥很难构思出来（思路难）；(1) 有些算法基本原理已经非常难懂，更别说亲自实现出来（实现难）。后者通过刷题可以逐渐克服，前者决定了你能否回答面试官问题。这个世界每时每刻，各种烧脑的算法被大牛们创造出来。即使翻过了世界上每一本算法书，还是会有你答不上来的问题，并且**在地球物理这个专业几乎没人思考这些**。
