...
作为 CS 这个学科里的过来人，我一直很想写一个什么东西来澄清一些见解，谈一谈程序员之间怎么拼内功。换一个方式来讲，我 CSers 固然都是写代码的，你地球物理学家也算是写代码的，此 “编程” 与彼 “编程” 有何本质上区别？

我系某大牛相信 “干地球物理的能当程序员使”，用来回答 “地球物理学位是不是与半个 CS 差不多？” 这个问题，其依据是前辈里有一位进游戏公司写代码。这句话不仅激怒了我，也让我开始重新地思考人生。按照我的理解，3D 建模软件中烟雾和流体特效主要靠纳维叶-斯托克斯方程来模拟，这位师兄搞流体力学的吧？

如此草率的断言，虽不是完全没有道理，恕在下直言一句：这两个学科之间的差别比大家想象得要大、相似度比大家想象得要低；跨行业申请的难度，比大家想象得要高。

#### —— 基本理论和性质 ——

开博第一篇，先跟大家交待我的学习背景。

在加拿大攻读计算机科学辅修学位，翻看过数学家 Kenneth H. Rosen 的数论教材，研究过包括 SICP 在内的计算机科学经典教材并因此而学了函数式编程 Common-Lisp（强制你形成递归的思维），Java 和 Python 也是我那时为了读算法书而学的… 对于这个领域内的各种出题套路和比赛规则，我都略有了解。

如果说要学好一门技能，九成靠的是自学的话，那往往那 10% 的引导和启发是最重要的。我那时所接触到的程序设计方法对我至今影响深刻，尽管这些训练只能让我成为一个新手，好在我走的是以算法为核心的正规化学习路线（Computer Science 并不是一个教你熟练掌握各种编程语言 + 开发框架的专业）。

在我看来，TopCoder / CodeForces 是一种对算法设计能力的训练 —— 软件是由更小的模块构成的，只有每一个细节都优化到位，才能提升整体的效率，算法比赛的重要性不容忽略。我虽代表不了这个学科的最高水平，希望以数独为例，引出算法比赛中的一些初等的优化方法。

![Algorithms 4th Edition](https://learning.oreilly.com/library/cover/9780132762564/250w/)

<small>普林斯顿大学的红皮「算法」（鄙人在加拿大时，这本书已经出到第四版，也是目前的最新版）</small>
...
看到这种摸不着边的公式兄弟们作何感想？不带一个积分符号、微分算子，小学生都能看懂，要想推导出它们可真不容易… 数论书里到处都是这种惊艳的结论。

带入标准数独的行列数 n=9，以上两式分别得 11988 和 10530<sup>[2]</sup>。相比于穷举法要快很多自不必说，甚至，这种解法的效率并不比后面要讨论的舞蹈链低。

[2]<small>这两个数字是针对每一个 9x9 的数独来说，需要尝试的循环次数，与上文 5475730538 这个数字含义不同。</small>

所以，从当前的方法下手多思考如何改进，总比靠硬件方法来加速更有研究价值。真想提速的话，用 C++ 重写所获得的效率提升都比用多进程明显。某些情形下，还得考虑进程之间的数据共享，搞不好依赖关系，容易出大问题。经验表明：假如一块 CPU 拥有 1024 个核，1024 个核同时工作，获得的效率提升远没有 1024 那么高。能有核数量的三分之一就很不错了，况且数据量的增长速度永远要比核心数的翻倍速度更快。**优秀的算法给你带来革命性的变化** —— 从 60 秒缩短到 2 秒、从 2 秒缩短到 10 毫秒… 这种感觉。
...
![Parallelized Quicksort](http://parallelcomp.uw.hu/files/09fig53.gif)

<small>并行版本的快速排序实现方法，转自 http://parallelcomp.uw.hu/ch09lev1sec4.html</small>

考虑到以上这些限制，算法比多进程拥有更大的应用范围和更直接的效应。假如在某种情形下我可以用 (1) 多进程和 (2) 更好的算法，我优先选择用算法来加速。毕竟这是各大公司录用 / 淘汰你的重要标准，算法方面的基本功不仅决定了你可以成为哪个档次的开发人员，更是**资深程序员用来显示逼格的一种资本**。
...
在 CS 这个学科里面，几乎哪里都离不开深搜遍历，这种算法几乎是万能的。求解数独问题，虽然不需要用多叉树这种数据结构，在 N 皇后 / 回溯法里面依然体现了深度优先搜索的思想。
...
链表是**软件工程里面常用的加速方法之一**。双向链表当中，每一个成员包含一个值和两个指向相邻成员的指针（Python 里没有指针，可以使用 attribute，下同)，且首尾相连形成环状结构；要想删除某一个成员，只需修改前、后两个成员的指针使得这个被删除成员在遍历时被跳过，便于对已删除成员快速恢复。
```Python
import collections
...
呼… 要想真正高效地求解一个数独问题，所需要的知识还挺多的！在我看来，这些数据结构非得要亲自实现一遍才能叫基本功过硬，这也是为什么很多人到了大二就放弃了。在滑铁卢大学读 CS，平均成绩只要够 80 就可以去 MIT 上研究生（全世界唯一一所享受如此待遇）。来到这里，各路算法比赛的大神居然会有 “觉得自己基础太差”、“读着吃力” 这种感觉… 何况是我等普通人？

可见编程其实很难，难在：(0) 有些数据结构在缺少背景知识的前提下，仅凭临场发挥很难构思出来（思路难）；(1) 有些算法基本原理已经非常难懂，更别说亲自实现出来（实现难）。后者通过刷题可以逐渐克服，前者决定了你能否回答面试官问题。这个世界每时每刻，各种烧脑的算法被大牛们创造出来。即使翻过了世界上每一本算法书，还是会有你答不上来的问题，并且**在地球物理这个专业几乎没有人思考这些**。
