---
layout:     post
title:      "3D 建模成果展示"
subtitle:   "用 3ds Max 重建 CS:GO 的武器模型"
date:       2019-11-24 12:00:00
author:     "__restrict"
header-img: "img/home-bg.jpg"
catalog: true
tags:
    - 3ds Max
    - Autodesk
    - UV mapping
    - Steam
    - Source SDK
---

**Guide**
* Polygons are saved as *.smd files
* Textures are saved as *.vtf files
* Final renders are saved as *.obj files

### Example Making

**The Workspace (P250)**
![workspace](/img/in-post/post-csgo-weapons/dem_lineswhite.PNG)

**Material Editor Window (AWP)**
![material](/img/in-post/post-csgo-weapons/Material_Editor.PNG)

---

### Автомат Калашникова (CV-47)

**Polygon**
![akpolygon](/img/in-post/post-csgo-weapons/ak47_in_3dsmax.png)

**Final Render with Lumination**
![akrender](/img/in-post/post-csgo-weapons/ak47_in_photoshop_72dpi_bicubic.jpg)

### M4A1 (Carbine)

**Polygon**
![m4polygon](/img/in-post/post-csgo-weapons/m4a1_in_3dsmax.png)

**Final Render with Lumination**
![m4render](/img/in-post/post-csgo-weapons/m4a1_in_photoshop_72dpi_bicubic.jpg)

### Arctic Warfare Magnum (AWP)

**Polygon**
![awppolygon](/img/in-post/post-csgo-weapons/awp_in_3dsmax.png)

**Final Render with Lumination**
![awprender](/img/in-post/post-csgo-weapons/awp_in_photoshop_72dpi_bicubic.jpg)

### Desert Eagle (Deagle)

**Polygon**
![deaglepolygon](/img/in-post/post-csgo-weapons/deagle_in_3dsmax.png)

**Final Render with Lumination**
![deaglerender](/img/in-post/post-csgo-weapons/deagle_in_photoshop_72dpi_bicubic.jpg)

---

**Programs Used**
* Steam and Source SDK
* GCFScape, Notepad++, StudioCompiler, VTFEdit, etc.
* Autodesk 3ds Max 2018
* Adobe Photoshop CC 2018

**Tutorials from**
* Dan Moughamian (Lighting in Adobe Photoshop)
* Matt Kloskowski (Lumination)
